...
def create_nearest_neighbor_path(start_city, dist):
    N = len(dist)
    ...
    distance = calc_total_distance(path, dist)
...
def worker_nn_start(dist, task_queue, result_queue):
    best_path_of_worker = []
    ...
    while True:
        ...
        current_dist, current_path = create_nearest_neighbor_path(
            start_city, dist)  # loop optimize
...
def solve(cities):
    ...
    for idx in range(num_cpu):
        process_list.append(multiprocessing.Process(
            ...
            args=(dist_list, task_queue, result_queue)))
